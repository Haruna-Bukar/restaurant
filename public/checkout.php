...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_order'])) {
    if (!isset($_POST['order_type']) || empty($_POST['order_type'])) {
        ...
        if ($_POST['order_type'] != "delivery") {
            header("Location: ../customer/process_order.php");
            exit();
        } else {
            header("Location: ../customer/delivery_form.php");
            exit();
        }
    }
}
...
function renderOrderSummary() {
    ...
    // Delivery fee: 500 if delivery, 0 otherwise
    ...
    // Sum subtotal and display items
    cart.forEach(item => {
        subtotal += item.price * item.qty;
        container.innerHTML += `<p><strong>${item.name}</strong> × ${item.qty} — ₦${(item.price * item.qty).toFixed(2)}</p>`;
    });

    const total = subtotal + deliveryFee;

    // Show subtotal, delivery, total
    container.innerHTML += `
        <hr>
...
// Update totals dynamically when order type changes
const orderTypeSelect = document.querySelector('select[name="order_type"]');
orderTypeSelect.addEventListener('change', renderOrderSummary);
</script>
